...
export default function TuningLayout({
  ...
  useEffect(() => {
    ...
    const hasCookie = document.cookie.includes('tuning_auth=');
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'linear-gradient(to bottom right, #f8fafc, #f1f5f9)', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      ...
      {isAuthenticated && <aside style={{ width: '256px', background: 'white', borderRight: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', position: 'fixed', height: '100vh', overflowY: 'auto' }}>
      ...
      {/* Main Content */}
      <main style={{ marginLeft: isAuthenticated ? '256px' : '0', flex: 1, overflow: 'auto' }}>
        {children}
      </main>
    </div>
  );
