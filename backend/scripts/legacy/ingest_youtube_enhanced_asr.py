...
def main():
    ...
    parser.add_argument('--days-back', type=int, help='Only process videos from last N days (default: all videos)')
    ...
    # Log days_back configuration
    if args.days_back:
        logger.info(f"Date filter: Processing videos from last {args.days_back} days")
    ...
    if args.channel_url:
        days_info = f" from last {args.days_back} days" if args.days_back else ""
        print(f"[FETCH] Fetching videos from channel{days_info}: {args.channel_url}")
        video_lister = YtDlpVideoLister()
        videos_data = video_lister.list_channel_videos(
            args.channel_url,
        ...
        print(f"[FETCH] Found {len(videos_data)} videos{days_info}, processing {len(video_ids)}")
    else:
